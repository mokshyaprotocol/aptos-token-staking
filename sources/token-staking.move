...
module mokshyastaking::tokenstaking
{
    ...
    use std::vector;
    struct TokenDPR has store, drop, copy
    {
        name:String,
        dpr:u64
    }
    ...
    public entry fun create_staking<CoinType>(
        ...
        aptos_framework::managed_coin::initialize<Coins>(
        &staking_treasur_signer_from_cap,
            coin_name,
            symbol,
            decimals,
            monitor_supply
    ...
    )acquires MokshyaStaking,ResourceInfo 
    {
        ...
        let l = vector::length(&staking_data.dpr_vec);
        let i =0;
        let dpr =0;
        while (i < l)
        {
            let val = *vector::borrow(&staking_data.dpr_vec,i);
            if (val.name == token_name)
            {
                dpr = val.dpr;
                break
            };
    ...
    public entry fun claim_reward<CoinType>(
        ...
        aptos_framework::managed_coin::mint<CoinType>(&staking_treasur_signer_from_cap,staker_addr,release_amount); 
    ...
    (   staker:&signer, 
        ...
        assert!(balanceToken>=tokens,ENO_INSUFFICIENT_TOKENS);
   ...
   fun test_create_staking(
       ...
       let receiver_addr = signer::address_of(&receiver);
        ...
        let token_dpr = vector<TokenDPR>[];
        vector::push_back(&mut token_dpr,TokenDPR{ name: string::utf8(b"Mokshya Token #1"),
        dpr:12} );
        ...
        // );
         ...
         let _staking_data = borrow_global<MokshyaStaking>(resource_address);
        // assert!(staking_data.state==false,91);
        // assert!(staking_data.amount==0,92);
    } 
    // #[test(creator = @0xa11ce, receiver = @0xb0b, )]
